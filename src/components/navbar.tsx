  ...
  return (
    <nav className="relative hidden lg:flex">
      {links.map(({ href, label, submenu }) => {
        ...
        const hasActiveSubmenu = submenu?.some(item => isLinkActive(item.href, pathname))
        
        return (
          <PlusGridItem key={href} className="relative flex items-center">
            {submenu ? (
              <Menu as="div" className="relative">
                <MenuButton className={`flex items-center justify-center px-4 py-3 text-base font-medium bg-blend-multiply data-hover:bg-black/2.5 ${
