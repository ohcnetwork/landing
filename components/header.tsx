...
export default function Header(props: {
    ...
    const [outDropdownData, setOutDropdownData] = useState<DropDownItem[] | null>(null);
    ...
    const productsItems = [
        {
            name: "CARE",
            description: "War ready EMR software, empowering thousands of ICU beds across India. All built on open source.",
            image: "/features/care-desktop.png",
            href: "/care"
        },
        {
            name: "Ayushma",
            description: "AI powered chatbot to assist doctors and nurses in managing patient care.",
            image: "/features/care-desktop.png",
            href: "/ayushma"
        },
        {
            name: "Leaderboard",
            description: "Tracking the progress of open source contributors and rewarding them for their contributions.",
            image: "/features/care-desktop.png",
            href: "/leaderboard"
        }
    ]
    ...
    useEffect(() => {
        setDropDownHeight(document.querySelector(".dropdown-animate-in")?.clientHeight || 0);
    }, [dropDownData])

    const NavigationItemRender = (props: { item: NavigationItem, onHover?: (hoverstate: boolean, leftOffset: number) => void }) => {
    ...
    const DropDownRender = (props: { items: DropDownItem[], className?: string }) => {
        const { items, className } = props;
        return (
            <div className={`flex items-stretch p-4 absolute top-0 inset-x-0 ` + className}>
                {items?.map((item, i) => (
                    <Link href={item.href} key={i} className="p-4 w-[200px] text-left rounded-lg hover:bg-black/10 transition-all flex flex-col gap-2">
                        <Image
                            src={item.image}
                            alt={item.name}
                            width={200}
                    ...
                    {navigation.map((item, i) =>
                        <NavigationItemRender
                            ...
                            onHover={(hoverstate, leftOffset) => {
                                if (hoverstate) {
                                    setShowDropdown(i + 1);
                                    triangleRef.current?.style.setProperty("left", `${leftOffset - triangleRef.current.clientWidth / 2}px`);
                                }
                            }}
                        />
                    )}
                    ...
                    <div className="mt-6 flow-root">
                        <div className="-my-6 divide-y divide-gray-500/25">
                            <div className="space-y-2 py-6">
                                {navigation.map((item, i) => <NavigationItemRender item={item} key={i} />)}
                            </div>
                        </div>
                    </div>
                </DialogPanel>
